...
export const PageLoadingState: React.FC<{ message?: string }> = ({
  ...
  return (
    <div className="min-h-[25vh] flex flex-col items-center justify-center p-4 text-center space-y-3">
      <div className="w-8 h-8 border-3 border-red-600/20 border-t-red-600 rounded-full animate-spin" />
      {message && <p className="text-xs font-mono text-zinc-500">{message}</p>}
    </div>
  );
};
